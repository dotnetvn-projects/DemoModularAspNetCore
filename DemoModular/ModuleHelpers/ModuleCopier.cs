...
namespace DemoModular
{
    ...
    {
        public static void CopyModules(this IHostingEnvironment env)
        {
            CopyModules(new string[] { @"../../DemoModular" });
        }

        public static void CopyModules(string[] args)
        {
            var workFolder = args[0];
            var moduleBuildPath = Path.Combine(workFolder, "DemoModular/Modules");
            ...
            foreach (var devFolder in moduleDevFolder)
            {
                ...
                foreach (var folder in folderCopy)
                {
                    ...
                    switch (copyPath.ToLower())
                    {
                        ...
                        case "wwwroot":
                            desPath = Path.Combine(workFolder, "DemoModular/wwwroot");
